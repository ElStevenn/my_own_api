...
_email_sender = email_sender.EmailSender() # This is not working! But i have to fix it!

app.mount("/static", StaticFiles(directory="static"), name="static")

# CORS origins setup (though you're allowing all origins in the middleware)
origins = [
    "http://localhost",
    "http://localhost:8000",
    "http://127.0.0.1",
...
app.add_middleware(
    CORSMiddleware,
    allow_origins=["*"],  # Allows all origins. Specify exact origins in production
...
# @app.post("/send_email")
# async def send_an_email(schemas.EmailBasicStructure):
#     """"""
#     return {"return": "Under construction"}
...
@app.get("/cv-es")
async def show_cv():
    file_path = os.path.join(os.path.dirname(__file__), "Pau_Mateu_Resume_esp.pdf") # Change this to the other resume
    return FileResponse(file_path)
...
# @app.post("/send_email", description="Method to send an email, this is an intern method")
# async def send_body_email(body_request: schemas.EmailBasicStructure)-> dict:
#     try:
#         _email_sender.send_email(body_request.to, body_request.subject, body_request.text or '')
#     except Exception as e:
#         return {"status":"error","response":f"An error ocurred {e}"}
#     return {"status":"sucsess","response":"Email has been sended succesfully"}
...
if __name__ == "__main__":
    ...
    uvicorn.run(
        ...
        ssl_keyfile = "/home/ubuntu/certificates/paumateu.key", # Place here ssl key file path
        ssl_certfile = "/home/ubuntu/certificates/www_paumateu_com.crt", # Place here ssl certificate path 
        reload=True
    )
